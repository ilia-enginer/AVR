...
// Вывод символа
static void ILI9341_WriteChar(uint16_t x, uint16_t y, unsigned char ch, FontDef font, uint16_t color, uint16_t bgcolor)
{
     uint32_t i, b, j;
...
void ILI9341_WriteString(uint16_t x, uint16_t y, const char* str, FontDef font, uint16_t color, uint16_t bgcolor)
{
		...
		while(*str)
		{
				...
				if ( (uint8_t)*str >= 0xC0 ){	// код 0xC0 соответствует символу кириллица 'A' по ASCII Win-1251
					...
					ILI9341_Draw_Char(buff_char, x, y, color, bgcolor, font);
				}
				else{
